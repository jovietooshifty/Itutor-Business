...
export function ImageCropModal({
  ...
  React.useEffect(() => {
    ...
    image.onload = () => {
      if (revoked) return
      /* Rejected, not upscaled. Stretching a 90px logo to 512 produces a
         blurry logo on every certificate and card, and the person who chose
         it has no way to tell that is what happened. */
      if (image.naturalWidth < spec.minWidth || image.naturalHeight < spec.minHeight) {
        setError(
          `That image is ${image.naturalWidth}×${image.naturalHeight}. It needs to be at least ` +
            `${spec.minWidth}×${spec.minHeight} — a smaller one would have to be stretched, and would look it.`
        )
        return
      }
  ...
  }, [file, spec.minWidth, spec.minHeight])
  ...
  function crop() {
    ...
    // Stage coordinates scale up to output coordinates by exactly this much.
    const ratio = spec.outputWidth / STAGE_WIDTH
    ctx.imageSmoothingQuality = 'high'
    ctx.drawImage(
      loaded.image,
      spec.outputWidth / 2 - (drawnWidth / 2 - offset.x) * ratio,
      spec.outputHeight / 2 - (drawnHeight / 2 - offset.y) * ratio,
      drawnWidth * ratio,
      drawnHeight * ratio
    )
    ...
    >
      ...
      >
        ...
        <p className="m-0 mt-1 text-xs text-ink-muted">
          Drag to move, and use the slider to zoom. Saved at {spec.outputWidth}×
          {spec.outputHeight}.
        </p>
